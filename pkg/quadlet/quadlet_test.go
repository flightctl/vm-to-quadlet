...
func TestConvert_BasicCompute(t *testing.T) {
	...
	t.Run("a per-VM .network unit is generated by default", func(t *testing.T) {
		requireFile(t, files, "myvm.network")
	})

	t.Run("default .network unit contains only a [Network] section", func(t *testing.T) {
		f := requireFile(t, files, "myvm.network")
		require.Contains(t, f.Content, "[Network]")
	})

	t.Run("Network= on the .pod unit references the per-VM .network unit by default", func(t *testing.T) {
		f := requireFile(t, files, "myvm.pod")
		require.Contains(t, f.Content, "Network=myvm.network")
	})
	...
	t.Run("no .network unit generated when Network= is set explicitly", func(t *testing.T) {
		o2 := opts()
		o2.Network = "host"
		files2, err := Convert(pod, o2); require.NoError(t, err)
		for _, f := range files2 {
			require.NotEqual(t, "myvm.network", f.Name, "unexpected .network unit when Network= is explicit")
		}
	})
